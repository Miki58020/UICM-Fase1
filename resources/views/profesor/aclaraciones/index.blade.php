...
@section('title', 'Mis aclaraciones | UICM')
...
<section class="bg-uicm-gray min-h-screen py-12 px-4"
    ...
    <div class="container mx-auto px-4 lg:px-12 max-w-7xl">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-2xl font-extrabold text-gray-900">Mis aclaraciones</h1>
        ...
        <div class="space-y-4" x-ref="lista">
            ...
            @php
                ...
                $paso2 = match($aclaracion->estado) {
                    ...
                    default     => ['color' => '#EFAD5A', 'texto' => 'En revisión','clase' => 'text-orange-500', 'icono' => 'dot'],
                };
            @endphp
            <div class="bg-white rounded-2xl shadow-md overflow-hidden" data-estado="{{ $aclaracion->estado }}">
                ...
                <div class="px-6 py-4 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0 self-start">
                        ...
                        <div class="w-8 h-0.5 mb-4" style="background-color: {{ $resuelta ? $paso2['color'] : '#E5E7EB' }};"></div>

                        <div class="flex flex-col items-center">
                            <div class="w-7 h-7 rounded-full flex items-center justify-center shadow-sm {{ !$resuelta ? 'ring-4 ring-orange-200' : '' }}"
                                 style="background-color: {{ $paso2['color'] }};">
                                @if($paso2['icono'] === 'check')
                                    <svg class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/>
                                    </svg>
                                @elseif($paso2['icono'] === 'x')
                                    <svg class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <p class="text-xs font-semibold mt-1 {{ $paso2['clase'] }} whitespace-nowrap">{{ $paso2['texto'] }}</p>
                        </div>
                    </div>
